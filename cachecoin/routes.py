from cachecoin.models import User
from cachecoin.forms import *
from flask import Flask, jsonify, request, render_template, url_for, flash, redirect
from cachecoin import app, db, bcrypt
from cachecoin import blockchainObj
...
def mine():
    ...
    if len(blockchainObj.pendingTransactions) == 1:  # Check if there is exactly one pending transaction
        ...
        if feedback:
            ...
            flash(f'Error!', 'danger')
    elif len(blockchainObj.pendingTransactions) > 1:  # Check if there are more than one pending transactions
        feedback = blockchainObj.minePendingTransactions(miner)
        if feedback:
            flash(f'Block Mined! Your mining reward has now been added to the pending transactions!', 'success')
        else:
